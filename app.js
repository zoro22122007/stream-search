...
function setMode(mode, btnId) {
    ...
    if (mode === 'mylist') {
        document.getElementById('sectionTitle').innerText = "Collection";
        document.getElementById('clearAllBtn').style.display = 'block';
        displayCards(getMyList());
    } else {
        document.getElementById('clearAllBtn').style.display = 'none';
        const titles = {'top/anime': 'Global Rated', 'top/manga': 'Top Manga', 'seasons/now': 'Trending'};
...
function displayCards(list) {
    ...
    list.forEach((item, index) => {
        ...
        card.innerHTML = `
            <button class="action-btn" onclick="handleAction(event, this, ${JSON.stringify({id, title, img, score}).replace(/"/g, '&quot;')})">
        ...
        card.onclick = (e) => {
            ...
            const targetUrl = currentMode.includes('manga') ? 'https://mangafire.to/home' : 'https://anikai.to/home';
...
document.getElementById('searchInput').addEventListener('input', (e) => {
    ...
    searchTimeout = setTimeout(async () => {
        const typeSearch = currentMode.includes('manga') ? 'manga' : 'anime';
        const res = await fetch(`${JIKAN_BASE}/${typeSearch}?q=${query}`);
        const data = await res.json();
        document.getElementById('resultsGrid').innerHTML = '';
        displayCards(data.data.filter(i => (i.title_english || i.title).toLowerCase().startsWith(query.toLowerCase())));
    }, 500);
});